...
const OpenLibraryScreen = ({ navigation }) => {
  const { colors } = useTheme();  // Extrai as cores do tema atual
  ...
  const fetchBooks = async () => {
    ...
    try {
      ...
      setBookList(response.data.docs); // 'docs' contém a lista de livros
    } catch (err) {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      />
      <Button title="Buscar" color={colors.primary} onPress={fetchBooks} />
      <Button title="Ir para Animes" color={colors.primary} onPress={() => navigation.navigate('Jikan')} />
